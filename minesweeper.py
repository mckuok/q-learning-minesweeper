...
class Minesweeper:
    ...
    def gameOver(self, won):
        for x in range(0, SIZE_X):
            for y in range(0, SIZE_Y):
                if self.tiles[x][y]["isMine"] is False and self.tiles[x][y]["state"] == STATE_FLAGGED:
                    self.tiles[x][y]["button"].config(image=self.images["wrong"])
                if self.tiles[x][y]["isMine"] is True and self.tiles[x][y]["state"] != STATE_FLAGGED:
...
class MinesweeperWrapper(threading.Thread):
    ...
    def run(self):
        ...
        self.minesweeper = Minesweeper(self.window)
